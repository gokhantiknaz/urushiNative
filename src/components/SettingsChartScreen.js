...
const DUMMY_DATA = [
    ...
    { powerp: 0, time: 1250, color: "chartreuse" }
];
...
const SettingsChartScreen = ({ data = [] }) => {
    ...
    useEffect(() => {
        // @ts-ignore
        const newPoints = DUMMY_DATA.map((point) => {
    ...
    useEffect(() => {
        //update limits
        if (points.values.length === 0 || points === undefined) return;
    ...
    const onTimeChange = (event, selectedDate) => {
        ...
        if (newTimeValue < 0 || newTimeValue > 1440) return;
